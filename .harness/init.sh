...
# init.sh nằm trong .harness/ → cd về ROOT project để chạy pnpm/docker đúng chỗ
cd "$(dirname "$0")/.."

echo "=== TaskFlow → CompanyOS: Harness Init & Verify ==="
echo ""

# 1. Confirm working dir (đã cd về root)
echo "=== [1/5] Working directory (root) ==="
pwd
...
echo ""
echo "Next steps:"
echo "1. Đọc .harness/feature_list.json — field current_focus = feature đang làm"
echo "2. Đọc .harness/CONTEXT.md để nắm lý thuyết/bài học chi tiết"
echo "3. Làm ĐÚNG 1 feature, verify (test/curl) TRƯỚC khi đánh done"
echo "4. Cập nhật .harness/progress.md + .harness/feature_list.json cuối session"
